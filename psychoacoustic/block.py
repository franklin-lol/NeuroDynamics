...
class Block:
    ...
    el_period:        float = 29.0     # elevation period for 3D (prime vs az)
    el_depth:         float = 0.35
    # Infra
    ...
    # "Дорогой" звук
    use_drone:        bool  = False    # detuned drone pad
    drone_vol:        float = 0.10
    drone_detune:     float = 0.15    # Hz
    use_wind:         bool  = False    # resonant wind pad
    wind_vol:         float = 0.07
    wind_bw:          float = 18.0    # BP bandwidth Hz
    use_lfo_filter:   bool  = False    # LFO filter sweep on binaural
    lfo_fc:           float = 900.0
    ...
    def render(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # ── LFO filter sweep (органическое дыхание сигнала)
        ...
        if self.infra_freq > 0:
            L, R = infra_modulate(L, R, self.infra_freq,
                                  self.infra_depth, self.seed*0.7)
